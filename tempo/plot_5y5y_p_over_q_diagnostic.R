## Temporary diagnostic: ratio between NGMMR physical and nominal-Q
## probabilities for true 5y5y forward-average inflation above 4%.
...
csv_file <- file.path(out_dir, "NGMMR_5y5y_P_over_Qn_gt4.csv")
pdf_file <- file.path(out_dir, "NGMMR_5y5y_P_over_Qn_gt4.pdf")
...
for (area in c("US", "EZ")) {
  ...
  ymax <- 1.12 * max(s_area$ratio_p_over_qn_5y5y, na.rm = TRUE)
  plot(s_area$date[s_area$psi == psi_values[1]],
       s_area$ratio_p_over_qn_5y5y[s_area$psi == psi_values[1]],
       type = "n", ylim = c(0, ymax), xlab = "",
       ylab = expression(P / Q[n]),
       las = 1, main = area_names[[area]])
  grid()
  for (i in seq_along(psi_values)) {
    idx <- s_area$psi == psi_values[i]
    lines(s_area$date[idx], s_area$ratio_p_over_qn_5y5y[idx],
